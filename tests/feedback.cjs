...
assert(await page.locator('#experience').count()===0,'satisfaction rating removed');await page.locator('#reply-body').fill('MNG đã xử lý');await page.locator('#reply-submit').click();await page.locator('#replies').filter({hasText:'MNG đã xử lý'}).waitFor();
