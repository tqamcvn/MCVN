...
await page.route('https://cdn.jsdelivr.net/**',route=>route.fulfill({contentType:'text/javascript',body:`window.testRows=[];window.testReplies=[];window.isManager=true;window.failSubmit=false;window.supabase={createClient:()=>({rpc:async(name,args)=>{if(name==='feedback_is_manager')return {data:window.isManager};if(name==='feedback_people')return {data:[{id:'test-user',display_name:'Test Manager',avatar_url:''}]};if(name==='feedback_set_status'){window.testRows.find(r=>r.id===args.target_id).status=args.new_status;return {data:null};}},storage:{from:()=>({createSignedUrl:async()=>({data:{signedUrl:'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aWV8AAAAASUVORK5CYII='}})})},auth:{getUser:async()=>({data:{user:{id:'test-user'}}})},from(table){let id;return {select(){return this},order:async()=>({data:table==='feedback_replies'?window.testReplies:window.testRows}),eq(k,v){id=v;return this},maybeSingle:async()=>({data:window.testRows.find(r=>r.id===id)||null}),then(resolve){resolve({data:table==='feedback'?window.testRows:[]})},insert:async row=>{if(window.failSubmit)return {error:'offline'};if(table==='feedback_replies'){window.testReplies.push({...row,created_at:new Date().toISOString()});return {};}window.testRows.push({...row,status:'Đang xem xét',created_at:new Date().toISOString()});return {}},delete(){return this}}}})};` }));
...
await page.locator('#received').click();await page.locator('#detail-error').filter({hasText:'Đã cập nhật: Đã nhận'}).waitFor();
await page.locator('#completed').click();await page.locator('#detail-error').filter({hasText:'Đã cập nhật: Hoàn thành'}).waitFor();
...
await page.evaluate(async()=>{window.isManager=false;await load();});await page.locator('.title-button').click();assert(!(await page.locator('#manager-actions').isVisible()),'member actions hidden');assert(!(await page.locator('#reply-form').isVisible()),'member reply form hidden');await page.locator('#detail-close').click();
