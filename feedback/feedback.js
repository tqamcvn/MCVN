...
function render(){
  ...
  filtered.forEach(r=>{
    ...
    const title=node('button',r.title,'title-button');title.onclick=()=>showDetail(r);content.append(meta,title,mentionParagraph(r.body,'excerpt'));if(r.attachments?.length)content.append(attachmentThumbnails(r.attachments));card.append(vote,content);$('list').append(card);
  });
}
function person(id){
...
async function setStatus(status){
  ...
  try{const result=await db.rpc('feedback_set_status',{target_id:id,new_status:status});if(result.error)throw result.error;await load();if(selectedFeedback.id===id){selectedFeedback=rows.find(r=>r.id===id);$('detail-meta').replaceChildren(person(selectedFeedback.author_id),node('span',' · '+selectedFeedback.kind+' · '),statusBadge(status));renderTask(selectedFeedback);$('detail-error').textContent='Đã cập nhật: '+status;}}
...
function mentionParagraph(body,cls){const paragraph=node('p','',cls);paragraph.append(feedbackMentionContent(body));return paragraph;}
